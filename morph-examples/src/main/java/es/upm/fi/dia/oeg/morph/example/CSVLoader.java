...
public class CSVLoader {
	...
	public static void main(String[] args) throws Exception {
        ...
        Connection conn = DriverManager.getConnection("jdbc:h2:~/morph-csv12", "sa", "");
        // add application code here
        //conn.setAutoCommit(true);

        
		CSVLoader.loadCSVFile(conn, csv_file);
		conn.close();
	}
    ...
    public static void loadCSVFile(Connection conn, String csv_file) throws Exception {
        ...
        try {
            ...
            conn.commit();
            logger.info("The table Sport created successfully");
        } catch (SQLException sqle) {
            logger.error("Error while creating the table Sport");
