...
class MorphMappingInferrer(mappingDocument:AbstractMappingDocument ) {
	val logger = Logger.getLogger("MorphMappingInferrer");
	var mapInferredTypes : Map[Node, Set[AbstractConceptMapping]] = null;
	var query:Query  = null;

	def addToInferredTypes(mapNodeTypes:Map[Node, Set[AbstractConceptMapping]] , node:Node , cms:Set[AbstractConceptMapping] ) 
	...
	def infer2(query:Query) : java.util.Map[Node, java.util.Set[AbstractConceptMapping]] = {
	...
	def infer() : Map[Node, Set[AbstractConceptMapping]] = {
	...
	def infer(query:Query ) : Map[Node, Set[AbstractConceptMapping]] = {
	...
	def infer(opQueryPattern:Op ) : Map[Node, Set[AbstractConceptMapping]] = {
		if(this.mapInferredTypes == null) {
	...
	def inferByRDFType(op:Op ) : Map[Node, Set[AbstractConceptMapping]] = {
		var mapNodeTypes:Map[Node, Set[AbstractConceptMapping]] = Map.empty;
	...
	def  inferSubjectTypesBySubjectURI(op:Op ) : Map[Node, Set[AbstractConceptMapping]] = {
		var mapNodeTypes:Map[Node, Set[AbstractConceptMapping]] = Map.empty;
	...
	def inferObjectTypesByExprList(exprList:ExprList ) : Map[Node, Set[AbstractConceptMapping]] = {
	...
	def inferObjectTypesByExpr(expr:Expr ) : Map[Node, Set[AbstractConceptMapping]] = {
		val mapNodeTypesExprs : Map[Node, Set[AbstractConceptMapping]] = {
			if(expr.isConstant()) {
	...
	def  inferObjectTypesByObjectURI(op:Op ) : Map[Node, Set[AbstractConceptMapping]] = {
		var mapNodeTypes : Map[Node, Set[AbstractConceptMapping]] = Map.empty
	...
	def  inferObjectTypesByPredicateURI(op:Op , mapSubjectTypes:Map[Node, Set[AbstractConceptMapping]] ) 
	: Map[Node, Set[AbstractConceptMapping]] = {
		var mapNodeTypes : Map[Node, Set[AbstractConceptMapping]] = Map.empty;
	...
	def inferSubjectTypesByPredicatesURIs(op:Op ) : Map[Node, Set[AbstractConceptMapping]] = {
		var mapNodeTypes : Map[Node, Set[AbstractConceptMapping]] = Map.empty;
	...
	def  getTypes(node:Node ) : Set[AbstractConceptMapping] = {
	  val result = {
	...
	def inferByURI(uri:String ) : Set[AbstractConceptMapping] = {
	  val cms = this.mappingDocument.getConceptMappings();
